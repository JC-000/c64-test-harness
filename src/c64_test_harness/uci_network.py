...
class UCIInterfaceAbsentError(UCIError):
    ...
    def __init__(self, identifier: int, cartridge_preference: str | None = None) -> None:
        ...
        super().__init__(
            ...
            f"Cartridge Preference = External ({pref}): measured on the "
            f"U64E (#359, paired ABBAAB, n=3 per arm), External left the "
            f"slot off the bus after a reset 3/3 while 'Command Interface' "
            f"still read Enabled, against Auto reading $C9 and completing "
            f"3/3. Under Auto the slot leaves the bus only if the firmware "
            f"detects a cartridge on the expansion port (source-read, "
            f"c64.cc ConfigureU64SystemBus); the RR-Net was on the port "
            f"throughout both of those arms and was not detected, so a "
            f"cartridge merely being present is not itself the cause. "
            f"Remedy: client.set_config_item('C64 and Cartridge "
