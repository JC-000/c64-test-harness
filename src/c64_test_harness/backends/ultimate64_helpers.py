...
# --------------------------------------------------------------------------- #
# Live memory watcher (pexpect-for-DMA)                                       #
# --------------------------------------------------------------------------- #
...
class ProgressEvent:
    ...
    :ivar kind: ``"Advanced"`` — at least one watched range changed since
        ...
        ``read_mem`` call raised; polling continues.
...
def watch_progress(
    ...
    *,
    ...
    slots, ...); this helper DMA-polls those ranges on a short interval
    and yields events when their bytes change, stall, or the wall-clock
    budget expires. See GitHub issue #108 for the motivating use case
    (multi-hour crypto handshakes on the U64E).
    ...
    existing :class:`Ultimate64Client` and any surrounding
    ``DeviceLock`` / debug capture context unchanged.

    Operationally:

    * Each poll issues one ``read_mem`` per watched name. DMA reads
      contend with the C64 bus, so ``poll_interval`` defaults to a
      conservative 10 s. Shorten it only for workloads where bus
      contention is known to be safe.
    * A read raising :class:`Ultimate64Error` (or any other exception)
      yields a ``"PollError"`` event and the generator KEEPS POLLING —
      one flaky read does not abort the watch. Persistent errors are
      visible as a stream of ``"PollError"`` events to the caller.
    ...
    * ``stop_when(values)`` is consulted after every successful poll;
    ...
    the U64 firmware's ``GET /v1/machine:readmem`` reads the address
    space as the 6510 currently sees it, with the active $01 banking
    applied. If the watched addresses fall in ROM-shadowed RAM, the
    caller MUST arrange for the C64 program to bank ROM off (write the
    appropriate value to $01) before the bytes of interest. This helper
    does NOT temporarily alter $01 — touching the banking register from
    the host would race the running program. Pick addresses that are
    visible under the running program's banking, or set up your code to
    keep ROM banked off across the watched region. Addresses below
    $A000 (e.g. the screen at $0400, low RAM at $0334 / $C000-$C3FF)
    are always RAM and need no special handling.
    ...
    def _gen() -> Iterator[ProgressEvent]:
        ...
        while True:
            ...
            for name, (addr, length) in plan:
                try:
                    data = client.read_mem(addr, length)
                except Exception as exc:  # noqa: BLE001
                    poll_error = exc
                    _log.warning(
                        "watch_progress: read_mem(%s @ 0x%04X, %d) raised %r",
