...
**On a real CS8900a** (external RR-Net on the U64E expansion port, live-verified fw 3.15) four things differ from VICE: set `C64 and Cartridge Settings -> Cartridge Preference = External` (on `Auto` the `$DE00` window reads zeros); start the program with `run_prg_via_sys(target, prg)` -- after `client.run_prg()` the cartridge is deselected, cause not isolated (#211); program the MAC from the 6510 with `cs8900a_set_mac_inline_code(mac)` -- host-side `set_cs8900a_mac` never reaches the expansion port (#209); and TX an ARP request before the first ping -- the harness routines never ARP and a macOS host queues replies behind a stale neighbour entry (#212). Full register-level detail in [docs/bridge_networking.md](docs/bridge_networking.md#real-silicon-diverges-from-vice-in-three-ways).
