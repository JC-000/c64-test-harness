...
**Two open defects in the TX builders** you should know before trusting a result from them: the `Rdy4TxNOW` poll is an unbounded hardware wait in every TX builder (nine emission sites, `bridge_ping.py:858`-`:2474`, all through one `_emit_tx_frame` helper), so a wedged chip hangs the 6510 with no diagnosis (issue #236), and `build_bridge_tx_code`'s result byte is a *completion* flag, not a *success* flag — it has reported thousands of successes with nothing on the wire (issue #235). There is also no way to reset the CS8900a and no probe that would catch it wedged (issue #234).
