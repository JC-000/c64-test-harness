...
**Host-side memory access does not reach the I/O chips.** `read_memory()` / `write_memory()` go through the binary monitor's `default` bank, which sees the RAM *under* the I/O window rather than the hardware: `read_memory(0xD012, 1)` returns a constant instead of a moving raster, `$D400-$D41F` returns RAM, and host writes to those addresses land in RAM too — so a SID or VIC configured host-side is never configured at all. Drive and sample the chips from 6502 code instead (write the registers, sample into a RAM buffer, call it with `jsr()`, read only the buffer); `tests/test_sid_emulation_live.py` is the worked example. This is worth knowing before writing a probe, because the host-side version does not error — it reports the same wrong answer for every configuration, including a healthy one.
