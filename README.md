...
**Known state on entry (opt-in, issue #227).** Setup is verifiable, teardown is not: a killed run restores nothing, and on a shared device the previous lane's turbo, REU size, SID map or `Cartridge Preference` is what the next lane inherits. With `U64_BASELINE_ON_ENTRY=1` (the one env name, read by the manager and by `HarnessConfig.from_env()`; or `HarnessConfig.u64_baseline_on_entry` from TOML `[u64] baseline_on_entry = true`, passed as `UnifiedManager(..., baseline_on_entry=...)`), every `acquire()` on the U64 backend runs `apply_factory_baseline(client)` right after the `DeviceLock` is taken and before the target is handed out: a per-category `PUT /v1/configs/<category>:reset_to_default` over the fourteen machine/SID/audio/drive/tape/printer/clock/LED/modem/UI stores (`BASELINE_CATEGORIES`), then an assertion that every item reads `current == default` — the firmware's own `default` from the item map, so there is no harness-owned table and a firmware release that moves a default is a deliberate bench change, not drift. **Reset, then assert:** drift found *before* the reset is the ordinary shared-device state and is logged per item at INFO ("inherited drift"); a mismatch *after* the reset means the reset did not take and raises `U64BaselineError` naming category, item, current and default. The reset is memory-only (flash untouched; a reboot reloads flash), and it never uses the global `configs:reset_to_default` route and never touches `Ethernet Settings`, `Network Settings` or the WiFi store (`BASELINE_EXCLUDED_CATEGORIES`) — the global route iterates those too and would flip a static-addressed device to DHCP. Categories the device does not list (the C64 Ultimate's set differs) are skipped with a log line. Six items are **detection-derived, not reset products** (`DETECTION_DERIVED_ITEMS`: `SID Sockets Configuration` → `SID Detected Socket 1/2`, `SID Socket 1/2`, `SID Socket 1/2 Capacitors` — measured `'8580'`/`'Enabled'`/`'22 nF'` against defaults `'None'`/`'Disabled'`/`'470 pF'` on the U64E with two 8580s fitted, identical across a physical power-cycle, n=1, while the other 197 of 203 items equalled their defaults): their category is still reset, they are reported under `report.detection_derived` with current and default, and never compared or PUT. Any other post-reset mismatch is still the hard failure, and its message says how to exempt one (`exempt=[(category, item)]` for a run; the pinned set, with a measurement, for good). Default is off, and off means no requests. `apply_factory_baseline(client, dry_run=True)` returns the `BaselineReport` (what drifted, what was reset, what still mismatches, what is detection-derived, what was skipped) without writing. `U64_BASELINE_ON_ENTRY` is the one switch for both the manager and `HarnessConfig.from_env()` (the prefixed `C64TEST_U64_BASELINE_ON_ENTRY`, which every field has by convention, wins if both are set). `snapshot_state`/`restore_state` stay as the courtesy on exit. Live coverage: `tests/test_entry_baseline_live.py` (`U64_BASELINE_LIVE=1`) and the flash-vs-default instrument `tests/test_flash_baseline_live.py` (`FLASH_BASELINE_LIVE=1`), both with `U64_HOST` and `U64_ALLOW_MUTATE=1`.
