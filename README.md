...
- **Queue-aware device locking** — `DeviceLock.acquire(timeout, *, progress_window=60.0)` extends the deadline while a live holder is making progress (only stuck/dead holders count against `timeout`); `create_manager(lock_timeout=...)` threads the timeout through `_LockedU64Manager` (long parallel benches use `lock_timeout=1800`); optional `c64-test-harness[notify]` extra adds `watchdog`-based fs-event wakeups (cooperative `os.utime` on release; 100 ms polling backstop preserved)
