...
> **Do not enable SocketDMA writes.** The write fast path is disabled pending a stability review; prefer `write_bytes` / `run_prg_via_sys` for bulk data (their 84-byte chunks stay on the non-leaking PUT path). The description below documents the mechanism for when it is re-enabled.
