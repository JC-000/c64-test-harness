...
ViceConfig(
    ...
    run_as_root=None,       # None = auto: Darwin + ethernet + /dev/bpf* unreachable
)
```

**Platform behavior — `run_as_root`**: Tri-state. `None` (the default) resolves to `True` only when all three hold: macOS, `ethernet=True`, and this process cannot open a `/dev/bpf*` node (`vice_lifecycle.bpf_capture_available()`). Stock macOS keeps those root-only, so elevation is the default there; a rig that runs `sudo chmod o+rw /dev/bpf*` makes them reachable and no sudo is used — an unprivileged VICE then attaches BPF and captures on `feth(4)` normally. Set explicitly to override. When True, `ViceProcess` launches x64sc via `sudo -n`, tracks the root-owned child via `_find_x64sc_child_pid()`, and routes `stop()` through `sudo -n kill` because an unprivileged parent cannot signal a root child; the NOPASSWD entry must name the exact x64sc path launched.

Do **not** restore blanket elevation on the strength of a `rawnet_arch_pre_reset+8` NULL deref during `cs8900_activate`. That crash is a VICE build without working ethernet (issue #144), not a permissions failure — it reproduces as root and with `/dev/bpf*` already `o+rw`. Homebrew's x64sc shows the subtler form: as root it starts, answers the monitor, and attaches **no** BPF device, so ethernet assertions pass against emulated registers with zero host traffic. See `docs/development.md` macOS caveats.
