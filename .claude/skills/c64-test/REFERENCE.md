...
- `BASELINE_ON_ENTRY_ENV` (`"U64_BASELINE_ON_ENTRY"`), `baseline_on_entry_enabled()`.
...
- `u64_baseline_on_entry` (default False) -- U64 reset-on-entry to factory default (issue #227). TOML `[u64] baseline_on_entry = true`; env `C64TEST_U64_BASELINE_ON_ENTRY=1`. Not wired automatically: pass `UnifiedManager(..., baseline_on_entry=cfg.u64_baseline_on_entry)` (the manager also reads `U64_BASELINE_ON_ENTRY` when the kwarg is `None`).
