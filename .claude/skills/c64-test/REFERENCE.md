...
- `BASELINE_CATEGORIES` — the fourteen covered stores: `C64 and Cartridge Settings`, `U64 Specific Settings`, `SID Addressing`, `SID Sockets Configuration`, `Audio Mixer`, `Drive A/B Settings`, `SoftIEC Drive Settings`, `Tape Settings`, `Printer Settings`, `Clock Settings`, `LED Strip Settings`, `Modem Settings`, `User Interface Settings`.
- `BASELINE_EXCLUDED_CATEGORIES` — `Ethernet Settings`, `Network Settings`, `WiFi settings`.
- `DETECTION_DERIVED_ITEMS` — frozenset of the six `SID Sockets Configuration` items the firmware derives from SID detection at boot (`SID Detected Socket 1/2`, `SID Socket 1/2`, `SID Socket 1/2 Capacitors`; U64E with two 8580s: `'8580'`/`'Enabled'`/`'22 nF'` vs defaults `'None'`/`'Disabled'`/`'470 pF'`, power-cycle-stable, n=1, 2026-09-05). Pinned literally by `tests/test_entry_baseline.py`; grows only with a measurement of that grade.
- `BASELINE_ON_ENTRY_ENV` (`"U64_BASELINE_ON_ENTRY"`, the one switch shared with `HarnessConfig.from_env`), `baseline_on_entry_enabled()`.
...
- `u64_baseline_on_entry` (default False) -- U64 reset-on-entry to factory default (issue #227). TOML `[u64] baseline_on_entry = true`; env `U64_BASELINE_ON_ENTRY=1` — the one name, which `from_env()` reads in addition to the convention form `C64TEST_U64_BASELINE_ON_ENTRY` (the prefixed one wins when both are set). Not wired automatically: pass `UnifiedManager(..., baseline_on_entry=cfg.u64_baseline_on_entry)` (the manager reads `U64_BASELINE_ON_ENTRY` itself when the kwarg is `None`).
