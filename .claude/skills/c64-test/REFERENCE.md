...
- `uci_socket_read(transport, socket_id, max_len=255, *, timeout=10.0, turbo_safe=False) -> bytes` — `max_len` above `SOCKET_READ_MAX_BYTES` (253) raises `ValueError`; one call drains one reply block and returns only the payload; `b""` when nothing is queued
