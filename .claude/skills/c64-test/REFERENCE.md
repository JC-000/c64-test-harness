...
- `run_prg_via_sys(target, prg, *, sys_addr=None, reset=True, boot_timeout=25.0) -> int` -- Load a PRG by `write_memory`, start it with a typed `SYS`, then resume; returns the entry address used. Takes a `TestTarget` or a bare transport; works on both backends. Exists because `Ultimate64Client.run_prg()`'s DMA load drops an external cartridge (`$DE00` reads zeros afterwards, issue #211). `reset=True` resets and waits for `READY.` (`TimeoutError` if it never appears); `sys_addr` defaults to the stub's own `SYS` (`ValueError` if none). Re-exported from the package root. Unit test: `tests/test_run_prg_via_sys.py`.
