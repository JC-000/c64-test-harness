...
**PRG/runner endpoints** (all use POST, not PUT — verified on 3.14; unchanged on the bench U64E 3.15 fork and the C64U 1.1.0, which the harness still drives with POST). **Every one of these carries the payload as an HTTP body, so on firmware predating GideonZ/1541ultimate#686 each call leaks one managed `/Temp` attachment — and `run_prg` leaks a second attachment for the sideload's own POST when its 404 fallback fires, since that re-sends the whole PRG body through an unchunked `write_mem` (`run_prg`'s 404 sideload path); whether the 404'd runner POST also leaves one is **unmeasured** (see PATTERNS for why the obvious experiment settles it in only one direction). A 404 from that endpoint is a wedge symptom, so the fallback doubles the cost exactly when the device is closest to the edge; pass `fallback_on_404=False` on a leak-prone device to see the 404 instead**, and enough accumulation **crashes the device firmware** — REST and the UCI bridge go down together and only a physical power-cycle recovers it. **"`/Temp` fills up" is the wrong model**: the C64 FPGA keeps running while the firmware stops answering both the network and the physical menu button, at ~5.8% RAM-disk usage (the disk is **16 MiB**: `ramdisk.cc` sizes it from `__ram_disk_start`=`0x02000000` / `__ram_disk_limit`=`0x03000000` in `target/u64{,ii}/riscv/ultimate/linker.x` at tag `1.1.0`; its "3 MB" comment is stale — issue #261). Trigger threshold and crash cause are both unestablished, and the count-versus-bytes question is **retired, not open** — both readings asked about capacity, and capacity is not what fails. Size conservatively as though attachments were counted, but that is a choice about which error to make, not a model; the only figure, "~15 cycles of a 63 KB PRG" (that module docstring in `backends/ultimate64_temp_gc.py`), was measured on the U64E on 3.14d with n unrecorded and is a budget for nothing. Note the GC cannot rescue you afterwards: **FTP is part of the firmware that crashes**, so prevention is the only option. `DeviceCapabilities.writemem_post_safe` is the per-device switch; see PATTERNS § "`/Temp` attachment hygiene" for the full leak/no-leak table and the budget.
