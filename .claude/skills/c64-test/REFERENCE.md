...
- `baseline_on_entry: bool | None` (U64 only, issue #227) — `True` runs `apply_factory_baseline(client)` on every `acquire()`, inside the `DeviceLock` and before the target is handed out; `None` (default) asks the environment (`C64TEST_U64_BASELINE_ON_ENTRY` wins, else `U64_BASELINE_ON_ENTRY`); unset/off means no requests. `HarnessConfig.u64_baseline_on_entry` is the TOML/env form and is tri-state — pass it here as is. A reset that does not take fails the `acquire()` with `U64BaselineError` (lock and device released). Refused with `RuntimeError` when `DeviceLock` is not importable: the entry reset never runs on a bare client.
...
- `BASELINE_ON_ENTRY_ENV` (`"U64_BASELINE_ON_ENTRY"`, the one switch shared with `HarnessConfig.from_env`), `baseline_on_entry_enabled()` — both resolved by `config.resolve_baseline_on_entry_env()` (`C64TEST_U64_BASELINE_ON_ENTRY` wins, else the shared name, else `None`).
