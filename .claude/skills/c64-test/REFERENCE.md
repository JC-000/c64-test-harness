...
- `baseline_on_entry: bool | None` (U64 only, issues #227/#285) — `True` runs `apply_factory_baseline(client)` on every `acquire()`, inside the `DeviceLock` and before the target is handed out; `False` opts out and means no requests at all. `None` (the default) is resolved **at acquire time, per device generation**: the environment first (`C64TEST_U64_BASELINE_ON_ENTRY` wins, else `U64_BASELINE_ON_ENTRY`, either direction), then `BASELINE_ON_ENTRY_DEFAULT_BY_GENERATION` — on for the U64E, off for the C64U, off for an unreadable generation (`unknown`, #262). It is **not** opt-in: a U64E lane that passes nothing gets the reset. Arming a `cbm` device explicitly logs a WARNING naming the WiFi-rejoin risk; the resolved value and what decided it are logged at INFO on every acquire. `HarnessConfig.u64_baseline_on_entry` is the TOML/env form and is tri-state — pass it here as is. A reset that does not take fails the `acquire()` with `U64BaselineError` (lock and device released). The entry reset never runs on a bare client, but the two ways of asking for it part company on a host where `DeviceLock` will not import: an **explicit** ask — `True` here, or `U64_BASELINE_ON_ENTRY=1` — is a hard refusal (`RuntimeError`, "baseline_on_entry requires DeviceLock"), while a reset merely **inherited** from the generation default degrades to off with a WARNING rather than turning a working configuration into a crash.

### `apply_factory_baseline(client, *, categories=BASELINE_CATEGORIES, dry_run=False, exempt=()) -> BaselineReport`
Module `backends.ultimate64_baseline`; package-root export. Reset-on-entry to the firmware's factory defaults (issue #227): per category present on the device, one category GET, one `PUT /v1/configs/<category>:reset_to_default` (`Ultimate64Client.reset_config_category_to_default`), then one item GET per item asserting `current == default` from the item map. **Reset, then assert** — pre-reset drift is logged at INFO per item ("inherited drift"), never raised; a post-reset mismatch raises `U64BaselineError` (an `Ultimate64Error`; `.mismatched` is `category -> item -> (current, default)`, `.report` the full report) after every category has been processed. Items without a `default` key are listed in `report.unasserted`, not compared. Items in the caller's `exempt=[(category, item), ...]` (the detection-derived class) are listed in `report.detection_derived` with `(current, default)`, never compared and never PUT — their category is still reset. A mismatch on any *other* item is the hard failure, and the message says how to exempt one for a run and where a hardware store belongs (`BASELINE_NEVER_TOUCH`). Categories the device does not list go to `report.skipped` with a log line. Never the global `configs:reset_to_default`; never a `BASELINE_NEVER_TOUCH` store — passing one (as a category or in `exempt`), or any glob (`*`/`?`), raises `ValueError` carrying the reason before any request. Memory-only: flash untouched. Calling it without holding the device's `DeviceLock` logs the #194 unlocked-client notice (same mechanism, not a second one).

- `BaselineReport` — `.drifted` / `.mismatched` (`category -> item -> (value, default)`), `.reset` (categories the firmware reports reset), `.skipped`, `.unasserted`, `.dry_run`; `.ok`, `.drifted_items()`, `.mismatched_items()`, `.summary()`.
- `BASELINE_CATEGORIES` — the covered stores: `C64 and Cartridge Settings`, `U64 Specific Settings`, `SID Addressing`, `Audio Mixer`, `Drive A/B Settings`, `SoftIEC Drive Settings`, `Tape Settings`, `Printer Settings`, `LED Strip Settings`, `Modem Settings`, `User Interface Settings`, and the C64U-only `Speaker Mixer` and `Keyboard Lighting` (#310; skipped on a device that does not list them). Per-generation item counts and their basis: `BASELINE_RECORDED_CATEGORY_SETS`.
- `BASELINE_NEVER_TOUCH` — `dict[category, reason]`, pinned literally by `tests/test_entry_baseline.py`: `Ethernet Settings`, `Network Settings`, `WiFi settings`, **`SID Sockets Configuration`** (a reset there cuts socket power and detection state — `u64_config.cc:744-800`, measured U64E n=3 — with the report reading clean; detection never re-runs over REST; recovery is a PUT of the detected values, which on a 6581 bench applies socket voltage without the human approval, `u64_config.cc:698-705`), **`Clock Settings`** (the RTC: reset shows no drift and no mismatch while arming the next PUT to write the 2015 defaults to the chip, `rtc.cc:350-409`). `BASELINE_EXCLUDED_CATEGORIES` is the tuple of its keys.
- `BASELINE_ON_ENTRY_ENV` (`"U64_BASELINE_ON_ENTRY"`, the one switch shared with `HarnessConfig.from_env`), `resolve_baseline_on_entry(generation, *, requested=None) -> (enabled, reason)`, `baseline_default_for_generation(generation) -> bool`, `baseline_on_entry_enabled(generation=None)`, `BASELINE_ON_ENTRY_DEFAULT_BY_GENERATION` (`{"ultimate": True, "cbm": False, "unknown": False}`; anything unlisted is off) — the env half is resolved by `config.resolve_baseline_on_entry_env()` (`C64TEST_U64_BASELINE_ON_ENTRY` wins, else the shared name, else `None`). Precedence: explicit `requested`, then the env var either way, then the generation default. Called with no generation, `baseline_on_entry_enabled()` answers the environment or off — the conservative reading for a caller with no device in hand.
- `Ultimate64Client.reset_config_category_to_default(category) -> list[str]` — the per-category route; returns the store names the firmware reports reset (empty when nothing matched). `reset_config_to_default()` (global) stays, documented as touching every store including the network ones.
...
- `reset(scope: str = "cpu", *, drive: str | int | None = None) -> None` -- `scope="cpu"` soft-resets the 6510; `scope="machine"` is the backend's fullest reset — deliberately loose cross-backend wording, since it is a VICE hard reset but on U64 a C64-level `reboot()` that leaves the firmware running (see `Ultimate64Transport.reset`); `scope="drive"` requires `drive=` (0..3 on VICE, "a"/"b" on U64).
...
- `read_memory(addr, length) -> bytes`
- `write_memory(addr, data, *, override=None) -> None` -- **Do not enable SocketDMA writes: the write fast path is disabled pending a stability review** (details tracked privately); use `write_bytes` / `run_prg_via_sys` for bulk data, and note there is no fast bulk-write path on a C64U today — the sanctioned replacement is ~128 128-byte PUT round trips for 16 KiB **on a C64U** (`write_bytes` chunks at the client threshold, #252). Measured on the U64E (fw 3.15, bce4535e, 2026-09-15, host on Wi-Fi (en0), link not instrumented, n=2-4 per arm, interleaved, #267), `write_bytes` runs about 1.3 KiB/s at 48-byte chunks and about 3.2 KiB/s at 128-byte chunks (median ~36-52 ms per PUT, observed 34-79 ms, dominated by the host link), and a single 16 KiB POST took ~0.1 s there; on a post-safe U64E `write_memory` itself is a single request. The rate is not measured on the C64U. What the disabled path does, for recognition: set `transport.socket_dma = True` (constructor kwarg or attribute; default False) and payloads >= `socket_dma_min_bytes` (default 8192) route via DMAWRITE on TCP 64 instead of REST (~150 ms vs >6 s for 16 KiB on C64U fw 1.1.0, whose POST writemem degrades at >=16 KiB). Same `MemoryPolicy` checks. DMAWRITE is fire-and-forget, so the write finishes with an in-band IDENTIFY completion barrier (FIFO command servicing means the reply proves every chunk was consumed and applied; recv timeout scales with payload size — same pattern as `reu_write`), then a REST tail read-back as a post-barrier sanity check (up to `socket_dma_verify_timeout`, 2 s). WARNING + REST fallback on connect/send/barrier/verify failure (connect failure latches the fast path off). A send or barrier failure is first retried once on a fresh connection (issue #223: the firmware closes a SocketDMA connection idle for >1 s and never reads what is written into it; the client also reopens a connection idle for `IDLE_RECONNECT_SECONDS` = 0.8 s before the next command, so the retry is the second line). Requires the device's DMA service on TCP 64 (C64U: enable Network Settings → "Ultimate DMA Service", ships disabled; present on the bench U64E's post-tag 3.15 fork — measured in issue #223; availability on 3.14 builds untested — connect failure just falls back to REST). The prohibition above covers this **bulk-write** path only; `socket_dma_reu_write` (REUWRITE) never consults the `socket_dma` switch and is not covered by it.
- `read_screen_codes() -> list[int]`
- `inject_keys(petscii_codes) -> None`
...
- `reset(scope="cpu", *, drive=None)` -- `scope="cpu"` → `client.reset()` (soft 6510); `scope="machine"` → `client.reboot()` (C64-level restart with cartridge/REU re-init, not a firmware reboot; ~8s before reachable); `scope="drive"` → `client.drive_reset(drive)` (drive `"a"` / `"b"` or `0` / `1`).
...
**Machine control:**
- `client.reset()` -- Soft reset the C64 (6510 CPU only; does NOT re-initialise cartridge/REU state)
- `client.reboot()` -- **C64-level reset, not a firmware reboot** (`C64::start_cartridge(NULL)`): the C64 side restarts with cartridge and REU re-initialised — which is why it clears REU/DMA stuck state and why it is required when switching turbo speeds with REU-heavy workloads — while the firmware keeps running, so firmware-RAM config, `/Temp` attachments and lwIP/UCI state all survive it. Allow ~8s settle.
...
- `client.reset_config_category_to_default(category) -> list[str]` -- `PUT /v1/configs/<category>:reset_to_default` (DESTRUCTIVE, memory-only; issue #227). Resets every store whose name pattern-matches `category` (glob, case-insensitive; a plain name is exact) and returns the store names the firmware reports as reset (`[]` when nothing matched). This is the primitive `apply_factory_baseline` uses; the global `client.reset_config_to_default()` iterates *every* store, `Ethernet Settings`/`Network Settings`/WiFi included — the Ethernet reset drops the DHCP lease mid-request and the WiFi reset re-effectuates the link a C64U is reached over.
...
- `restore_speed_defaults(client, *, defaults=None) -> dict[str, str]` -- Pass `defaults=` the `{item: default}` you already read and validated (e.g. at fixture entry, so a missing default refuses before any write) and it reads nothing, writing exactly those values; it must name exactly `CPU Speed` and `Turbo Control` with non-empty strings, else `ValueError` before any write (#370). The speed restore: writes `CPU Speed`, then `Turbo Control`, back to the `default` each item reports. Reads both first (bodyless GETs), then one query-string PUT per item — zero `/Temp` attachments on any grade; never the body-carrying `set_config_items_batch`. Use it wherever a module that changed speed puts the device back: `set_turbo_mhz(client, None)` leaves `CPU Speed` behind and `set_turbo_mhz(client, 1)` leaves `Turbo Control = Manual`, both `!= default` (#360, #365). Both items are written unconditionally (on a U64E the entry baseline leaves them clean and the drift comes later). Raises `Ultimate64ProtocolError` before writing anything when an item has no non-empty default; otherwise attempts both writes and raises `Ultimate64RestoreError` naming every item that failed. Returns `{item: value}` as written. Package-root export. Not a replacement for `snapshot_state`/`restore_state`, which restore entry values and stay as they are (#365).
- `max_cpu_speed_mhz(client) -> int` -- The device's maximum turbo speed from the same preset probe (64 on C64U fw 1.1.0, 48 on the U64E — 3.14 and the bench 3.15 fork alike; 48 fallback when inconclusive). Backs `Ultimate64Transport.set_speed(None)`. Module-level export, not re-exported from the package root.
...
- `snapshot_state(client) -> U64StateSnapshot` -- Capture turbo + REU + cartridge + badline + bus-mode + SID-addressing + `Cartridge Preference` state (the last since #221 — it is memory-only, survives `machine:reboot`, and used to leak between lanes). The later fields default to `""`, and `restore_state` skips empty values, so snapshots taken before they existed still restore.
- `restore_state(client, snap)` -- Restore a snapshot. Orders the `Cartridge` item first in its config batch — the same abort-before-half-applied invariant as `set_reu` (a firmware rejection of the Cartridge write aborts the batch before the REU is half-enabled) — and `Cartridge Preference` last (its PUT is what re-selects the expansion port, #217).
- `CARTRIDGE_SETTINGS_CATEGORY` / `CARTRIDGE_PREFERENCE_ITEM` -- the `"C64 and Cartridge Settings"` / `"Cartridge Preference"` strings the RR-Net recipe and the #217 re-PUT use (issue #221). Package-root exports; `CARTRIDGE_SETTINGS_CATEGORY` is the same string as `CAT_CART`.
- `reset(client)` -- Soft reset (CPU only)
- `reboot(client)` -- C64-level reset with cartridge/REU re-init, not a firmware reboot (clears REU/DMA stuck state, ~8s settle)
...
**Do not enable SocketDMA writes — the write fast path is disabled pending a stability review** (details tracked privately). Use `write_bytes` / `run_prg_via_sys` for bulk data.

Binary "SocketDMA" command channel on TCP port 64, distinct from REST. Covers capabilities REST lacks (keyboard inject, REU write, raw reset, DMA load/jump) and is the transport behind the `Ultimate64Transport.socket_dma` bulk-write fast path. Wire format: 2-byte LE opcode + 2-byte LE length + payload; mostly fire-and-forget (no reply — confirmation is the connection staying open), so pair every write with a read-back where correctness matters. Commands on one connection are serviced strictly in order, so a replying command (IDENTIFY) doubles as a completion barrier — `reu_write` does this by default, and the transport's `DMAWRITE` bulk-write fast path finishes with the same barrier before its tail read-back sanity check. The device closes a connection idle for >1 s (`SO_RCVTIMEO` in `socket_dma.cc`), so `SocketDMAClient(..., idle_reconnect=IDLE_RECONNECT_SECONDS)` (0.8 s; `None` disables) reopens a reused connection before the first command after such a gap — `idle_reconnects` counts them, `idle_seconds` reports the current gap. On the C64 Ultimate the service ships disabled (enable Network Settings → "Ultimate DMA Service"); present on the bench U64E's post-tag 3.15 fork (issue #223), untested on 3.14 builds. Both classes are package-root exports.
...
**Rate cap — read this before designing any turbo-speed test that relies on the trace.** The U64E FPGA emits the debug stream at a fixed rate of **~1.02M entries/sec** (2,842-2,843 UDP packets/sec on the NTSC U64E at 1 MHz, wired host, #432 — consistent with one entry per NTSC phi2 cycle: a rate match, 360 entries/packet, n=3, Debug Stream Mode presumed at default 6510 Only, not recorded) regardless of CPU turbo speed. This matches the native 6510 rate at 1 MHz, so at 1 MHz the trace is essentially complete. At higher turbo speeds you get a **uniformly sampled 1/N view** of the real bus (1/4 of cycles at 4 MHz, 1/48 at 48 MHz). Turbo speed itself adds no sequence gaps at any speed, because the rate limit is at the source, not in the UDP path. **`packets_dropped` is not zero in general** (#424): the bench's own loss ran 0.4-45% of packets per 1 s capture, placed at the host's Wi-Fi downlink, load-dependent, with a small device-side residual not excluded (#356). Drop to 1 MHz for the capture window with `restore_speed_defaults(client)` if you need a complete trace (`set_turbo_mhz(client, 1)` also runs at 1 MHz but writes `Turbo Control = Manual`, which is off its default — #365); turbo-speed capture is only sound for uniform-sample aggregate statistics (PC-hit distribution, frequency maps). Measurement lives in `tests/test_u64_debug_stream_speed_live.py`.
...
# Multicast: join towards the device (#399, received on a wired host); the
# default INADDR_ANY join lets the kernel pick (the VPN here, which WARNs).
...
- `get_uci_enabled(client) -> bool` / `enable_uci(client)` / `disable_uci(client)` — config-side helpers, take an `Ultimate64Client`. Bodyless config PUTs (`set_config_items`), so they cost no `/Temp` attachment on any firmware
- `UCIInterfaceAbsentError(identifier, cartridge_preference=None)` — a `UCIError`, exported at the package root (PR #394, #359). On an `Ultimate64Transport`, `_execute_uci_routine` reads the UCI identifier at `$DF1D` before every routine: one bodyless GET, so zero `/Temp` cost on any firmware. If the byte is not `$C9`, it raises before anything is written or typed, instead of timing out at the sentinel. Attributes: `identifier` (the byte read) and `cartridge_preference` (the item's current value from one more bodyless GET, or `None` if that read failed). The usual cause is `Cartridge Preference` = External (see SKILL pitfall 15); the harness never changes it for you. `uci_probe` does not pre-check (`check_identifier=False`), because reporting the identifier is its job. Other transports skip the read. A `$C9` shows only that the slot is on the bus; it does not rule out a STATE-bit wedge (`uci_wedge_probe`)
...
- `build_uci_command(target=3, cmd=2, params=b"", resp_addr=..., status_addr=..., resp_len_addr=..., stat_len_addr=..., error_addr=..., sentinel_addr=..., code_addr=..., turbo_safe=False) -> bytes` — a `target` with the no-reply bit (`$80`) raises `ValueError`, since every routine waits for a reply after the push (#486); with `turbo_safe=True` the builder still emits a fifth `params` byte (515 B), but at the default `$C000` that routine reaches the `$C200` reply area and `_execute_uci_routine` refuses it, so 4 is the practical limit
...
- ARP defaults (`None`) emit no ARP code (pinned by SHA-256 in `tests/test_cs8900a_arp.py`, re-taken when #487 added the TX skip phase). ARP on makes them larger — consume 738 B, responder 783 B, TOD responder 907 B, ping-and-wait 472 B, TOD ping 596 B — so size code windows accordingly. Proven under VICE and on the simulated chip (`tests/cs8900a_sim.py`). The ARP ping path has run on the U64E, on the pre-#487 routine (`tests/test_first_exchange_live.py`; #222 on 2026-09-05, #444/PR #458 on 2026-09-16); the responders' `my_mac` ARP answer is still unmeasured on hardware.
