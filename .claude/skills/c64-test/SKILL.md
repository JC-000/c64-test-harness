...
6. **`wait_for_text()` works with binary transport** — it calls `resume()` between polls internally, so no workaround is needed. Pass `verbose=False` (default) or `verbose=True` for debug output. It also resumes in a `finally`, so the CPU is running on **every** exit path — match, timeout or exception. `ScreenGrid.from_transport()` does **not** resume; a poll loop built out of it never advances the C64 and a running machine is indistinguishable from a hung one.
