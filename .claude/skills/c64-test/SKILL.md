...
13. **Hardware RR-Net on the U64 is not the VICE bridge** — set `Cartridge Preference = External` (`client.set_config_item("C64 and Cartridge Settings", "Cartridge Preference", "External")`), start PRGs with `run_prg_via_sys` (never `client.run_prg`, which drops the cartridge), program the MAC from the 6510 with `cs8900a_set_mac_inline_code` (`set_cs8900a_mac` works under VICE and is useless on hardware), send an ARP request before the first ping to a host, and use the `*_tod_code` builders via `run_subroutine` — `run_ping_and_wait`/`run_icmp_responder` need `jsr()` and are VICE-only. PATTERNS § "Hardware RR-Net on the U64".
