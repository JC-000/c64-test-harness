...
- Cost: one category GET + one reset PUT + one item GET per item per category — **151 items across the twelve covered categories, measured on the U64E (fw 3.15) 2026-09-12** (read-only, `DeviceLock` held, bodyless GETs). `tests/test_entry_baseline_live.py` records `apply_seconds`. The same read counted 40 items in the five `BASELINE_NEVER_TOUCH` stores and 12 more in two categories that are in neither list (`UltiSID Configuration` 8, `Data Streams` 4), for **203 across all 19 categories the device lists**. So the covered figure and the all-category figure differ by scope, not by disagreement — the "~150" that circulated for months was accurate and was withdrawn only because nobody had measured it. **Unexplained residual:** #276 counted **201** all-category items on 2026-09-10 and this read counts **203** on 2026-09-12, same device. Two items appeared. Nobody has established why — device state and a different counting basis are both open, neither is evidenced — so both figures are recorded with their dates rather than averaged or chosen between. **These are U64E figures only** — the C64U's category and item lists have never been read, so it does not carry over to the CBM line. Every one of those requests is bodyless, so the reset costs **zero `/Temp` attachments** even on the leak-prone C64U; the residual is that the zero-length gate (`routes.cc:40-46`) was read on the 3.15 line and 1.1.0 is an inference, not a read.
