...
**Accumulated attachments fill `/Temp`, and a full `/Temp` crashes the firmware.** The owner, 2026-09-15: "When /Temp is filled then the device firmware crashes. the c64 appears to continue to run but the rest api becomes unresponsive and the local firmware menu switch no longer has an effect. This is the outcome of the writemem garbage collection issue on unpatched firmware." So the machine looks alive (the C64 FPGA keeps running) while the **device firmware is dead**: it stops answering the network *and* stops responding to the physical **menu button** on the case. **Nobody knows how many uploads an unpatched device survives**, and no count is stated, guessed or budgeted against here.

The size of that RAM disk is established: **16 MiB** — `ramdisk.cc` takes its size from the linker symbols `__ram_disk_start` (`0x02000000`) and `__ram_disk_limit` (`0x03000000`) in `target/u64/riscv/ultimate/linker.x` and `target/u64ii/riscv/ultimate/linker.x` at tag `1.1.0`; the "3 MB" in that file is a stale trailing comment and was repeated across this repo (issue #261). It does not translate into a count of uploads: how much of it a given workload fills before the crash has never been measured.

So size the budget conservatively, as though attachments were counted: that is a choice about which error to make, not a measured limit. The earlier guessed wedge count (one U64E on 3.14d, n unrecorded) is **retired** (owner, 2026-09-15): do not cite it, and do not size a budget against it.

**Two consequences that change what the hygiene is for.** First, **FTP cannot recover a wedged device**: the FTP server is part of the firmware that has crashed, so `gc_temp_folder` is unavailable exactly when attachments matter most. Prevention is not the better option here, it is the only one. Second, because a `reboot()` does not delete attachments either (measured — see the recovery section below), the accumulation you are spending against is whatever the device has held since its last successful GC or power-cycle, the previous lane's residue included.
...
**This boundary is measured, not inferred** (C64 Ultimate at fw 1.1.0 — product `"C64 Ultimate"`, fpga 122, core 1.49, `writemem_post_safe=False`, `write_mem_query_threshold=128` — 2026-09-10, under the `DeviceLock`, with FTP as the instrument between every step, **n=1 per arm**, writes to `$C000`):

| Write | Path taken | `/Temp` managed attachments after |
|---|---|---|
| baseline, before anything | — | 0 |
| `write_mem` 64 bytes | `PUT ?data=` | **0** |
| `write_mem` **128** bytes (exactly at the ceiling) | `PUT ?data=` | **0** |
| `write_mem` **129** bytes | `POST` + body | **exactly 1** (`temp0000`) |
| then `gc_temp_folder(keep=0)` | FTP | 0 — `ok=True deleted=['temp0000'] error=None` |

So: the ceiling is **exact and inclusive** (128 is still PUT), the POST path leaks **exactly one attachment per call**, and `gc_temp_folder` works end-to-end against a C64U over anonymous FTP. n=1 per arm is deliberate and sufficient — this is a deterministic code-path question, not a flaky-hardware one — but it is n=1; do not cite it as more.

**Every write in that table was read back and byte-compared before the `/Temp` count was taken**, and any future re-measurement must do the same: "no attachment appeared" and "the write never happened" are indistinguishable without the readback, and that is exactly how a regression here would hide.
...
| `build_ping_and_wait_code` | **336** | plain — no ARP, no drain | **yes** |
...
| `build_ping_and_wait_tod_code` | **460** | plain | **yes** |
...
**Caveat on the SocketDMA row — reasoned from the code path, not measured.** It is the one row in that table still resting on reasoning rather than on a device measurement. `SocketDMAClient` speaks a raw binary protocol on TCP 64 that never enters the firmware's HTTP server, and the attachment is created by that server's body/attachment handling, so it should not be able to leak. That is an argument from the code (`u64_socket_dma.py`; firmware `software/network/socket_dma.cc`) and from the scope `ultimate64_temp_gc.py` states for the leak — it has **not** been confirmed by counting `/Temp` before and after a SocketDMA run on a device. Treat it as strong but unverified; do not rest a bulk-write budget on it alone. And note that a SocketDMA write which **falls back to REST** (connect, send, barrier or verify failure — it logs a WARNING) is a POST, and does leak.
...
- Cost: one category GET + one reset PUT + one item GET per item per category, so the item GETs one run makes are the covered total for the device's generation in `BASELINE_RECORDED_CATEGORY_SETS` (`ultimate64_baseline.py`) — the one table of per-category item counts and covered / never-touch / neither / all totals, each count with its basis, firmware and date. It is cited here by name and not restated: while the figure lived in prose it moved four times in three days ([#342](https://github.com/JC-000/c64-test-harness/issues/342)). Its U64E counts are device-read and match firmware source; its C64U counts are source-derived only — the C64U's category list was read once (2026-09-15, 20 categories, one bodyless `GET /v1/configs`, fw 1.1.0, [#287](https://github.com/JC-000/c64-test-harness/issues/287)) but its item lists never have, so no U64E figure carries over to the CBM line. `tests/test_entry_baseline_live.py` records `apply_seconds`, and `tests/test_entry_baseline_table_live.py` (`BASELINE_TABLE_LIVE=1`, read-only, bodyless GETs) compares a device against its record. The unexplained residual between #276's compared-item count and the U64E record ([#292](https://github.com/JC-000/c64-test-harness/issues/292)) is kept in that record, not resolved. Every one of those requests is bodyless, so the reset costs **zero `/Temp` attachments** even on the leak-prone C64U; the residual is that the zero-length gate (`routes.cc:40-46`) was read on the 3.15 line and 1.1.0 is an inference, not a read.
- **Why the C64U defaults off anyway, deliberately and not by oversight.** Not `/Temp` — the link. That device is reached over WiFi whose reconnection after a power cycle is known unreliable, with nobody present, and nothing in `BASELINE_NEVER_TOUCH` protects against a future edit adding a network store to the covered set by mistake. `U64_BASELINE_ON_ENTRY=1` opts a C64U in for somebody at the bench, and the manager logs a WARNING naming the risk when it does. `tests/test_ultimate64_transport_live.py`'s `speed_baseline` fixture is deliberately stricter and does not contradict this: it resolves the same switch but never resets a device not graded `ultimate`, even with `U64_BASELINE_ON_ENTRY=1` (it logs a WARNING that the reset was refused), because it calls `apply_factory_baseline` by hand and that function has no generation gate of its own (#276). An `unknown` generation (unreadable or timed-out probe, #262) resolves off for the same reason: never arm a reset on a device the harness failed to identify. `HarnessConfig.u64_baseline_on_entry` stays `None`-by-default (tri-state, "nobody asked") so wiring the config through does not turn an inherited default into an explicit request (`config.py`; the contract is in the `ultimate64_baseline` module docstring, pinned by `tests/test_entry_baseline_default_on.py`).
...
> **Do not enable SocketDMA writes.** The write fast path is disabled pending a stability review. For bulk data use `write_bytes` / `run_prg_via_sys`, whose threshold-sized chunks stay on the PUT path on every grade (#252). Details are tracked privately.
...
On leak-prone firmware this loop is also the wedge shape: one `/Temp` attachment per `run_prg`, and a reboot between speeds does not collect them. Count the uploads a parametrized sweep will make before you run it, and see § "`/Temp` attachment hygiene" above.
...
The `multicast_group=` argument on `DebugCapture` is the portable receive path — the U64's default `Stream Debug to` destination is the multicast group `239.0.1.66:11002`. Unicast (`<local-ip>:11002`) only works when the Mac/Linux host and the U64 share an L2 segment. **A multicast join receives only when it is made towards the device** (#399, U64E fw bce4535e, 2026-09-23, host wired on the device's LAN, n=3 per arm): `device_host=` (or an explicit `multicast_interface=`) on `AudioCapture`/`DebugCapture`/`VideoCapture` received the group in every arm, while the default INADDR_ANY join received 0 in 6/6 because the kernel routes `239.0.1.x` via the VPN on this bench; a default join whose route is a tunnel now logs a WARNING naming it (#494). Over Wi-Fi no join received anything (#461), so unicast is the fallback there.
...
**`runner_health_check()` is itself a body-carrying call** — it POSTs that tiny no-op PRG, so on leak-prone firmware every probe costs one more attachment. Never poll it in a retry loop on such a device: a health check used to "wait for the runner to come back" is the fastest way to add attachments to the `/Temp` that fills and crashes the firmware.
...
Route unwanted SID engines to Unmapped addresses (e.g. `$D500`) so they don't contribute audio. "SID Socket" config is just Enabled/Disabled; chip type is firmware auto-detected (read-only). **Disable "SID Player Autoconfig" and "Auto Address Mirroring"** before mapping multiple SIDs to adjacent addresses. SID address routing changes take effect on the config PUT with **no reset** (measured #204, U64E fw 3.15, 27/27 across three interleaved rounds — the old "reset + ~3 s settle" advice was never measured); `isolated_sid_addressing()` reads the whole map back and raises on mismatch. **Digital audio stream does NOT reflect Audio Mixer panning** (analog-only) — use frequency analysis (Goertzel DFT) not stereo separation for multi-SID validation.
