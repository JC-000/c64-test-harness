...
- Cost: one category GET + one reset PUT + one item GET per item per category — **151 items across the twelve covered categories, measured on the U64E (fw 3.15) 2026-09-12** (read-only, `DeviceLock` held, bodyless GETs). `tests/test_entry_baseline_live.py` records `apply_seconds`. The same read counted 40 items in the five `BASELINE_NEVER_TOUCH` stores and 12 more in two categories that are in neither list (`UltiSID Configuration` 8, `Data Streams` 4), for **203 across all 19 categories the U64E lists, 2026-09-12**; the 151, 40, 12 and 203 figures were also reproduced from firmware source at `7f6fcb51` (v3.15-85) in #288's adversarial review, each store's `t_cfg_definition[]` preprocessed with the U64E build flags and the five REST-visible item types counted. So the covered figure and the all-category figure differ by scope, not by disagreement — the "~150" that circulated for months was accurate and was withdrawn only because nobody had measured it. **Unexplained residual ([#292](https://github.com/JC-000/c64-test-harness/issues/292)):** #276 records "201 items compared" (category scope and firmware build not recorded) on the U64E on 2026-09-10, and this read counts **203** on the U64E on 2026-09-12. The candidates are a different counting basis or a different firmware build: firmware source rules out drift within one build — store items are appended only when a store is constructed, and the REST listing emits every item of the five value types — except for stores registered at runtime (the monitor-bookmarks store, the per-device SID stores). Nobody has established why, so both figures are recorded with their dates rather than averaged or chosen between. **These are U64E figures only** — the C64U's category list was read once (2026-09-15, 20 categories, one bodyless `GET /v1/configs`, fw 1.1.0, [#287](https://github.com/JC-000/c64-test-harness/issues/287)) but its item lists never have, so the U64E figures do not carry over to the CBM line. Every one of those requests is bodyless, so the reset costs **zero `/Temp` attachments** even on the leak-prone C64U; the residual is that the zero-length gate (`routes.cc:40-46`) was read on the 3.15 line and 1.1.0 is an inference, not a read.
