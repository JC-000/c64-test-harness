...
- **Six items are detection results, not reset products.** `SID Sockets Configuration` → `SID Detected Socket 1/2`, `SID Socket 1/2`, `SID Socket 1/2 Capacitors` read what the firmware found in the sockets at boot (`'8580'`/`'Enabled'`/`'22 nF'` on the U64E with two 8580s; defaults `'None'`/`'Disabled'`/`'470 pF'`), unchanged across a physical power-cycle (n=1, 2026-09-05). `DETECTION_DERIVED_ITEMS` exempts them from the comparison — still reset with their category, reported under `report.detection_derived`, never PUT. A mismatch on any other item stays a hard failure; the message tells you `exempt=[(category, item)]` for one run and the pinned set (with a measurement) for good. Without the exemption every SID-fitted device would fail entry with a false "reset did not take".
