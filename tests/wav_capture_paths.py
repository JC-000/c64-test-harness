...
def capture_dir(
    ...
    :raises ValueError: if *suite* is empty or not a single plain
        path component, or if the destination
        ``<scratch>/wav_captures/<suite>`` resolves to the tracked
        directory or anywhere inside :data:`TRACKED_ROOT` -- which
        catches a scratch root under the tracked tree, ``tests/`` itself
        (whose ``wav_captures/<suite>`` *is* the reference), and symlinks
        to either.  The helper, not pytest's ``basetemp``, is what keeps a
        default run out of the reference tree.
    """
    ...
    dest = Path(scratch) / "wav_captures" / suite
    resolved = dest.resolve()
    tracked = (TRACKED_ROOT / suite).resolve()
    if resolved == tracked or TRACKED_ROOT in resolved.parents:
        raise ValueError(
            f"destination {dest!s} resolves to {resolved!s}, inside the tracked "
            f"capture tree {TRACKED_ROOT!s}; set {REFRESH_ENV}=1 to refresh the "
            f"reference deliberately, otherwise use a scratch root outside it"
        )
    return dest
