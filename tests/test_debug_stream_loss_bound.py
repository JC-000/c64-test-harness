"""The debug-stream loss bound sits between the bench's measured loss and a broken receiver.
...
* **Below it:** the worst loss measured on the bench, 0.446
  (U64E fw 3.15 bce4535e, 2026-09-15, 28 captures; kernel socket-buffer
  drops zero in the 12 bracketed; loss at the host's Wi-Fi downlink,
  load-dependent, device-side residual not excluded -- see the live
  comment).  A measured figure, restated from the comment
...
**Offline residual (X1).** This module checks the helpers, not that the live
test calls them.  A live assertion edited to bypass ``_debug_loss_within_bound``
at its call site passes every test here; only a live run can catch it.
"""
...
def test_the_bound_admits_the_worst_measured_bench_loss() -> None:
    assert MEASURED_BENCH_LOSS_MAX <= DEBUG_STREAM_LOSS_MAX
...
def test_the_bound_rejects_a_byte_swapped_sequence_number() -> None:
    ...
    packet.  The bound must sit below that, with the bench max below it,
    ...
    assert MEASURED_BENCH_LOSS_MAX < DEBUG_STREAM_LOSS_MAX < loss
