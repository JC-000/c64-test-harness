...
| `$C100-$C3E1` | 738 | `bridge_ping.run_ping_and_wait / run_icmp_responder` | TX / echo-match / echo-respond routines (largest: the ARP-answering echo-respond routine of run_icmp_responder(my_mac=...), 738 bytes; 429 without my_mac) | consume_addr= |
| `$C400-$C402` | 3 | `uci_network.build_socket_write (also build_socket_read's multi-block routine`, `$C400-$C401)` | 16-bit inner-loop countdown (lo, hi) + Y save slot across the turbo fence; the multi-block read's 16-bit store countdown | hardcoded |
