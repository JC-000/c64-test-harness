...
| `$0334-$033B` † | 8 | `backends.ultimate64_probe.probe_u64` | check_write=True only (#241): 8 bytes read, overwritten with their inverse by a query-string PUT writemem (no body, no /Temp attachment), read back, then written back and verified; an unconfirmed restore logs a WARNING | hardcoded |
