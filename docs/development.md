...
- On macOS, VICE's `pcap` ethernet driver requires **root**, and `/dev/bpf*` permissions are irrelevant to that. The gate is VICE's own: `archdep_rawnet_capability()` (`src/arch/shared/archdep_rawnet_capability.c`) returns `geteuid() == 0` on macOS — the `CAP_NET_RAW` branch is Linux-only — and `rawnetarch.c` admits the pcap driver only when it holds. Unelevated, `rawnet_arch_driver` is never assigned and stays NULL, and `cs8900_activate` derefs it. (An earlier version of this note blamed a root-only macOS 26 kernel attach check inside `pcap_open_live()`; VICE never gets that far. Verified live: `/dev/bpf0` at `crw----rw-`, uid 501, driver still refused.) The observed symptom is a SIGSEGV at `rawnet_arch_pre_reset+8` inside `cs8900_activate`:
  ...
  - `ViceConfig.run_as_root: bool | None = None` auto-detects: it elevates when `ethernet=True` and the configured driver is one VICE gates behind `archdep_rawnet_capability()` (pcap — i.e. every macOS launch) and this process does not already hold that capability. The gate is `geteuid() == 0` plus a Linux-only `CAP_NET_RAW` branch; `/dev/bpf*` permissions are **not** consulted by VICE, and the former `bpf_capture_available()` heuristic has been removed. Linux's `tuntap` driver is ungated, so Linux needs no elevation.
  - When elevation is needed but `sudo -n` is not authorised for the exact binary, `ViceProcess.start()` raises `ViceElevationRequiredError` **before spawning anything**. What an operator should read and paste is the exception's printed message (`str(err)`, and what the elevation gate's session-end notice shows verbatim): it renders the elevated launch with any `-addconfig` temp-rc path replaced by a placeholder — that file is a per-launch temp file `ViceProcess` deletes the moment this refusal unwinds (`stop()`'s `_cleanup_tmp_vicerc()`, called from `start()` on any failure), so the raw path is already gone by the time the message is read — and names `.sudoers_entry` as the durable fix. `.command`/`.argv` carry the exact, real argv instead, for a caller that wants to re-run the launch **programmatically**, right now: they are only valid before that cleanup runs, so treat them as immediate-use-only, not something to log or hand an operator to paste later. Unelevated, VICE would SIGSEGV in `rawnet_arch_pre_reset()` with no log output. `run_as_root=False` on a launch that needs root is refused for the same reason; `VICE_ETHERNET_ALLOW_UNELEVATED=1` opts out.
  - `ethernet=True` also requires the resolved binary to be built with raw-network support, and `resolve_vice_executable()` **probes** rather than assumes: it runs `x64sc -features` (which prints the compile-time table and exits — no emulator) and reads `HAVE_RAWNET` / `HAVE_PCAP` / `HAVE_TUNTAP`, cached per binary path+mtime+size. It raises `ViceEthernetBinaryError` if the binary lacks rawnet, or if `ethernet_driver` names a driver the build does not carry (also a NULL-driver SIGSEGV). A binary that cannot be exec'd falls back to an image scan, which sees rawnet but not which drivers, and therefore never refuses a driver.
  - **The PATH binary is the normal case.** Homebrew's 3.10 bottle reports `HAVE_RAWNET yes` / `HAVE_PCAP yes` (`HAVE_TUNTAP no`) and links libpcap; nothing about ethernet work needs a separately-built VICE on macOS. `$VICE_ETHERNET_BIN` (or `ViceConfig(ethernet_executable=...)`, whose default reads it) remains as an **override** for a bench that must point elsewhere — leave it unset by default. There is no TOML key for it; `HarnessConfig` has no `vice_ethernet_executable` field and `_from_dict` drops unknown keys silently. (Issue #144's "Homebrew has no working ethernet" line was a misreading of the unelevated crash.)
  ...
  - The NOPASSWD entry (when elevation does fire) must name the **exact** x64sc path being launched. The harness resolves a bare `x64sc` through `PATH` before handing it to sudo — sudoers matches the absolute command path, so a rule can only ever name one, and Linux sudoers commonly set a `secure_path` that omits Homebrew-style prefixes (stock macOS sets none: `sudo -n -l` lists no `secure_path` Default and `sudo -n x64sc -features` exits 0 there, so an earlier claim that the bare name "fails under sudo on macOS" was wrong) — but deliberately does **not** resolve symlinks: sudoers matches the literal command path, so authorise `/opt/homebrew/bin/x64sc`, not the `/opt/homebrew/Cellar/vice/3.10/bin/x64sc` it points at. `ViceElevationRequiredError.sudoers_entry` emits exactly that form.
  - `tests/bridge_platform.py:probe_vice_pcap_ok()` applies the same elevation rule, so the probe measures the configuration the tests will run under; when elevation is unobtainable it skips with an actionable reason rather than launching a VICE that would crash. It also requires the probed VICE to hold a real `/dev/bpf*` handle: reaching the binary monitor is **not** proof of capture, because a VICE whose rawnet driver never attached still emulates the CS8900 registers and would let the ethernet suite pass vacuously (issue #144).

  The `sudo -n` wrap is **non-interactive**: it requires a passwordless sudoers entry for `/opt/homebrew/bin/x64sc` on macOS dev hosts. See the "Passwordless sudo for bridge lifecycle" subsection above — the same sudoers drop-in that covers the three bridge scripts now also needs an `x64sc` entry. `scripts/verify-dev-env.sh` parses the `NOPASSWD:` rules out of `sudo -n -l` (the same way `vice_elevation.parse_sudo_listing` does) and reports a `warn` row if no rule names `/opt/homebrew/bin/x64sc` exactly — a blanket `(ALL) ALL` does not count, because it still prompts. (It used to probe with `sudo -n -l <cmd>`, which exits 0 for anything a `(ALL) ALL` user may run at all, so it reported `ok` on hosts where every elevated launch prompted.)
...
Two env vars, both defined in `tests/conftest.py`, turn a silent skip into a hard failure so a missing prerequisite on CI or a fresh bench cannot pass a mocks-only run as green:

- **`C64_REQUIRE_VICE=1`** — declares that this invocation must exercise a real `x64sc`. Any `@pytest.mark.vice_live` test that would skip for lack of an emulator fails instead, and if the whole run ends without a single live VICE test actually executing (wrong `-k`, an `--ignore`, a stray future `skipif`), the session is failed and escalated to exit status 1 at `pytest_sessionfinish`. Leave it unset for ordinary development runs — the bare skip is the default, and the reason is visible under `-rs`.
- **`C64_REQUIRE_ELEVATION=1`** — the same idea for the elevated prerequisites host-side ethernet/bridge tests need: a NOPASSWD sudoers rule for the exact `x64sc` path (`elevation("vice_root")`), NOPASSWD rules for the bridge lifecycle scripts (`elevation("bridge_scripts")`), a usable `/dev/bpf*` node (`elevation("bpf_nodes")`, macOS only), or the bridge interface(s) being present and up (`elevation("bridge_iface")`). Tests carry `@pytest.mark.elevation(kind, **kwargs)`; each kind is probed at most once per session (cached) and, unset, a missing prerequisite is a bare skip whose reason **is** the remedy (the exact `sudo`/`visudo` line, or `sudo chmod o+rw /dev/bpf*`) — no `-rs` needed to see it, because `pytest_sessionfinish` always prints an `ELEVATION REQUIRED: N test(s) skipped` section listing every distinct kind, its count, and its remedy when anything was skipped for elevation. Set the var and a missing prerequisite fails at setup instead (same remedy text), and a green exit status is escalated 0 → 1 (never masking an already-worse INTERRUPTED=2 or INTERNAL_ERROR=3). A mid-fixture refusal — `ViceProcess.start()` raising `ViceElevationRequiredError` because a preflight probe was bypassed with `MACOS_PCAP_ENABLED=1` — is converted the same way by `start_vice_or_skip()`, so it shows up in the same notice.

Run both together for a bench that is supposed to be fully wired (e.g. before trusting a "the ethernet suite is green" claim):
